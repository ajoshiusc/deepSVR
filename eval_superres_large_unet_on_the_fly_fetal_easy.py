...
MODEL_FILE = '/project/ajoshi_27/code_farm/deepSVR/model_64_unet_large_lrem4_hcp_easy/epoch_2600.pth'
...
stack1 = stacks.cpu().numpy()[:, 1]
stack2 = stacks.cpu().numpy()[:, 2]
stack3 = stacks.cpu().numpy()[:, 3]
stack4 = stacks.cpu().numpy()[:, 4]
stack5 = stacks.cpu().numpy()[:, 5]
...
for i in range(batch_size):
    ...
    write_nifti(stack1[i],'stack1'+str(i)+'.nii.gz')
    write_nifti(stack2[i],'stack2'+str(i)+'.nii.gz')
    write_nifti(stack3[i],'stack3'+str(i)+'.nii.gz')
    write_nifti(stack4[i],'stack4'+str(i)+'.nii.gz')
    write_nifti(stack5[i],'stack5'+str(i)+'.nii.gz')
