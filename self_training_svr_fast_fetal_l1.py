...
def masked_l1(moving, slicevol):
    msk = slicevol > 0
    err = torch.abs(moving-slicevol)*msk
    loss = torch.sum(err)
    return loss
    
image_loss = masked_l1
...
write_nifti(recon_image[0,0],f'outsvr_fast_fetal_l1/deepsvr_recon_orig.nii.gz')

write_nifti(image[0,0],'outsvr_fast_fetal_l1/deepsvr_orig.nii.gz')
...
for epoch in range(max_epochs):
    ...
    if np.mod(epoch, 10) == 0:
        ...
        torch.save(reg.state_dict(),'./outsvr_fast_fetal_l1/epoch_reg_'+str(epoch)+'.pth')
        torch.save(superres.state_dict(),'./outsvr_fast_fetal_l1/epoch_superres_'+str(epoch)+'.pth')

    #vol_loss.backward()


        write_nifti(recon_image[0,0],f'outsvr_fast_fetal_l1/deepsvr_recon_{epoch}.nii.gz')
