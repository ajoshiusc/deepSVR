...
class Model(nn.Module):
    def __init__(self, reg, img):
        super(Model, self).__init__()

        self.regnet =  reg
        self.recon_img = nn.Parameter(img)
    def forward(self, slice_vol):
        ddf = reg(torch.cat((self.recon_img, slice_vol), dim=1))
        recon_image_moved = warp_layer(self.recon_img, ddf)

        return recon_image_moved, self.recon_img
...
svr_model = Model(reg, recon_image)
optimizer_svr = torch.optim.Adam(svr_model.parameters(), 1e-15)
...
for epoch in range(max_epochs):
    ...
    for d in tqdm(range(6)):
        ...
        for sliceno in range(int(64/2)):
            ...
            recon_image_moved, recon_img = svr_model(slice_vol)
