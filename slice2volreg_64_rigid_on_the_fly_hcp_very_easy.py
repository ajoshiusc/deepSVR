...
sublist_full = glob('/home/ajoshi/HCP_1200/*/T1w/T1*.nii.gz')
...
trained_model_file = './model_64_slice2vol_reg_hcp/epoch_'+str(start_epoch)+'.pth'
...
randstack_transforms = Compose(
    [
        ...
        # make stacks
        RandAffined(mode=("bilinear"), prob=1.0, translate_range=(1, 1, 1), rotate_range=(
            np.pi/20, np.pi/20, np.pi/20), padding_mode="zeros", keys=["image"]),
...
for epoch in range(start_epoch+1, max_epochs):
    ...
    if np.mod(epoch, 10) == 0:
        ...
        torch.save(model.state_dict(),
                   './model_64_slice2vol_reg_hcp_easy/epoch_'+str(epoch)+'.pth')
