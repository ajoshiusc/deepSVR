...
trained_model_file = './model_96_slice2vol_reg_fetal/epoch_'+str(start_epoch)+'.pth'
...
randstack_transforms = Compose(
    [
        ...
        Resized(keys=["stack"], spatial_size=[96, 96, 96]),
...
for epoch in range(start_epoch+1, max_epochs):
    ...
    for batch_data in train_loader:
        for sliceno in range(int(96/4)):
            ...
            fixed = torch.zeros(batch_size, 1, 96, 96, 96)
    ...
    if np.mod(epoch, 10) == 0:
        ...
        torch.save(model.state_dict(),
                   './model_96_slice2vol_reg_fetal/epoch_'+str(epoch)+'.pth')
        valid_loss = 0
        ...
        for valid_batch_data in valid_loader:
            for sliceno in range(int(96/4)):
                ...
                valid_fixed = torch.zeros(batch_size, 1, 96, 96, 96).to(device)
                slice_ind = torch.tensor(range(4*sliceno, 4*(sliceno+1)))
