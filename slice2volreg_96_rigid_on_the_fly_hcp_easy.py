...
trained_model_file = './model_96_slice2vol_reg_hcp/epoch_'+str(start_epoch)+'.pth'
...
randstack_transforms = Compose(
    [
        ...
        Resized(keys=["stackx","stacky","stackz"], spatial_size=[96, 96, 96]),
...
resize_x_down = Resize(spatial_size=[int(96/slice_thickness), 96, 96])
resize_y_down = Resize(spatial_size=[96, int(96/slice_thickness), 96])
resize_z_down = Resize(spatial_size=[96, 96, int(96/slice_thickness)])
resize_up = Resize(spatial_size=[96, 96, 96])
...
for epoch in range(start_epoch+1, max_epochs):
    ...
    for batch_data in tqdm(train_loader):
        for sliceno in range(int(96/slice_thickness)):
            ...
            slice_vol = torch.zeros(batch_size, 3, 96, 96, 96)
    ...
    if np.mod(epoch, 10) == 0:
        ...
        torch.save(model.state_dict(),
                   './model_96_slice2vol_reg_hcp_easy/epoch_'+str(epoch)+'.pth')
    print(f"epoch {epoch + 1} average loss: {epoch_loss:.4f}")

    np.savez('svr_reg_epoch_loss_values_on_the_fly_hcp_easy.npz',
