...
randstack_transforms = Compose(
    [
        ...
        ScaleIntensityRangePercentilesd(
            keys=["image"], lower=2, upper=99, b_min=0.02, b_max=0.99, clip=True),
        # make stacks
...
for epoch in range(max_epochs):
    ...
    if np.mod(epoch, 10) == 0:
        torch.save(model.state_dict(),
                   './model_64_unet_large_lrem4_hcp_perceptual_loss_easy/epoch_'+str(epoch)+'.pth')
    ...
    np.savez('large_unet_on_the_fly_loss_values_hcp_perceptual_loss_easy.npz',
